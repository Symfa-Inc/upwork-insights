...
class CityProcessor:
    ...
    def get_similar(
        ...
        final_result = [(city.capitalize(), score) for city, score in final_result]
        return final_result
...
class CountryProcessor:
    ...
    def get_similar(self, name: str, threshold: float = 0.75) -> List[Tuple[str, float]]:
        normalized_name = self._normalize(name)
        similarities = []
        for country, iso2, iso3 in self.database:
            scores = [
                (country, ratio(normalized_name, country)),
                (country, ratio(normalized_name, iso2)),
        ...
        filtered_results = [item for item in similarities if item[1] >= threshold]
        return sorted(filtered_results, key=lambda x: x[1], reverse=True)[:1]
...
if __name__ == '__main__':
    ...
    test_country_names = ['RUS', 'USA', 'Dnemark', 'Danmark', 'Denmarc', 'DEN']
